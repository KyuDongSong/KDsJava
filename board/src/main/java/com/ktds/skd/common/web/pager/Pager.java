package com.ktds.skd.common.web.pager;
...
public abstract class Pager {
	...
	/**
	 * 전체 게시물의 수 검색을 할 경우, 검색된 게시물의 수
	 */
	private int totalArticleCount;

	/**
	 * 한 페이지에 노출할 게시글의 수 기본값 10(변경 가능)
	 */
	protected int printArticle;

	/**
	 * 한 페이지 그룹에 노출할 페이지의 수 기본값 10
	 */
	int printPage;

	/**
	 * 어떤 페이지 번호에서 노출할 게시글의 시작 번호 ex) 1page 시작번호 1번 // 2page 시작번호 11번
	 */
	...
	public void setTotalArticleCount(int count) {
		...
		this.totalPage = (int) Math.ceil((double) this.totalArticleCount / this.printArticle);
		this.totalGroup = (int) Math.ceil((double) this.totalPage / this.printPage);
	}
